...
$(document).ready(function () {
  ...
  $('#tabelaProdutos').DataTable({
    ...
    ajax: {
      type: "POST",
      url: "../../php/controller/produto/produto.Listar.php",
      data: { data: 0 },
      dataType: "json",
    ...
    oLanguage: {
      ...
      sInfo: "Quantidade de produtos: _TOTAL_",
  ...
  $('#tabelaProdutos tbody').on('click', 'tr', function () {
    ...
    const registro = $('#tabelaProdutos').DataTable().rows('.selected').data();

    // Mostra quantidade de registros selecionados
    const qtd_selecionada = $('#qtdSelecionadoProd');
    ...
    const btn_adicionar = $('#btnAdicionarProd');
    ...
    const btn_alterar = $('#btnAlterarProd');
    if (registro.length === 1) {
    ...
    const btn_deletar = $('#btnDeletarProd');
    if (registro.length > 0) {
  ...
  $('#reseta_modal_add_prod, #fechar-add-prod, #fechar-alt-prod, #canc_delete').on('click', function () {

    $('#add-produto, #altera-produto, #deleta-produto').each(function () {
      this.reset();
    });

    const form_alterar = $('#altera-produto')[0];
    $(form_alterar[4]).removeClass('readonly');

    // Reseta msg de info status e todos inputs marcados como ínvalidos
    $('#add-produto, #altera-produto').removeClass('was-validated');
  });

  // deixa invisível o label e input para adicionar mais unidades ao produto
  $('label[for="id-add-mais-unid-produto"], #id-add-mais-unid-produto').attr({ 'disabled': false }).hide()
  ...
  */
  $(document).on('click', '#btnAdicionarProd, #cliqueAqui', function () {

    const registro = $('#tabelaProdutos').DataTable().rows('.selected').data();

    // ----------------  ANÁLISE ---------------------
    const form_adicionar = $('#add-produto')[0];
    ...
    if (registro.length == 1) {
      ...
      $('#id-add-tipo-produto').val($(`option:contains("${registro[0][3].trim()}")`).val());     // tipo produto

      form_adicionar[3].min = form_adicionar[3].value;
      $('#reseta_modal_add_prod').hide();
      ...
      $('label[for="id-add-imagem-produto"], #id-add-imagem-produto').attr({ 'disabled': true }).hide()
    ...
    // Apaga os dados que estão no formulário 
      ...
      $('label[for="id-add-imagem-produto"], #id-add-imagem-produto').attr({ 'disabled': false }).show()
  ...
  * setar os dados nos inputs do formulário e também apaga-los quando necessário.  
  */
  $(document).on('click', '#btnAlterarProd', function () {
    ...
    // Seta o tipo do produto selecionado nas opções contidas no select 
    $('#id-alt-tipo-produto').val($(`option:contains("${registro[0][3].trim()}")`).val());     // tipo produto

    $(form_alterar[4]).attr('readonly', 'readonly');
 ...
 */
  $(document).on('click', '#btnDeletarProd', function () {
    const registro = $('#tabelaProdutos').DataTable().rows('.selected').data();

    $('#deleta-produto').each(function () {
      this.reset();
    });
  ...
  $('#check-desativar-produto').on('click', (e) => {
