...
$nome            = addslashes(trim($all_datas[0]['add-nome-estoque']));
$marca           = addslashes(trim($all_datas[0]['add-marca-estoque']));
$preco           = addslashes(trim($all_datas[0]['add-preco-estoque']));
$custo           = addslashes(trim($all_datas[0]['add-custo-estoque']));
$quantia         = addslashes(trim($all_datas[0]['add-quantidade-estoque']));
$quantia_minima  = addslashes(trim($all_datas[0]['add-quantidade-minima-estoque']));
$tipo            = addslashes(trim($all_datas[0]['add-tipo-estoque']));
...
if(empty($all_datas[1])){
    ...
    $imagem = $imgDir . md5($all_datas[0]['add-nome-estoque']) . '.jpg';
}
...
//if (move_uploaded_file($_FILES['imagem']['tmp_name'], $imgDir . $imagemF)) {
    ...
    if(isset($all_datas[0]['add-mais-unid-estoque'])){
        $entrada = new Entrada();
        $unidades = addslashes(trim($all_datas[0]['add-mais-unid-estoque']));
